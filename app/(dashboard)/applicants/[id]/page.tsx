...
export default function ApplicantProfilePage() {
  ...
  const [schedule, setSchedule] = useState({
    round: "CULTURE_FIT", scheduledAt: "", format: "Google Meet", durationMins: 45,
  });
  ...
  async function parseCv() {
    ...
    try {
      ...
      setParseError("Cannot connect to server. Check your connection and try again.");
    } finally {
      setParsing(false);
    }
  }
  ...
  if (!candidate) return <p className="text-slate-500">Loading...</p>;
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="card col-span-2 space-y-3">
          ...
          <p className="text-xs text-slate-500">
            Open PDF → Cmd+A → Cmd+C → paste text below:
          </p>
          <textarea
            className="input h-40"
            placeholder="Paste CV text here for AI to extract skills and experience..."
            value={cvText}
            onChange={(e) => {
          ...
          />
          <button className="btn-primary" onClick={parseCv} disabled={parsing || !cvText.trim()}>
            {parsing ? "⏳ Analyzing (15–30 sec)..." : "✨ Parse CV"}
          </button>
          {parseError && (
          ...
          {parseSuccess && (
            <p className="text-sm text-green-600 bg-green-50 rounded p-2">
              ✅ Parsed successfully! Profile has been updated.
            </p>
          )}
        ...
        <div className="card space-y-2">
          ...
          <textarea className="input h-16" value={noteText}
            onChange={(e) => setNoteText(e.target.value)} placeholder="Add a note..." />
        ...
        <div className="card space-y-4">
          ...
          {(application.stage === "CULTURE_FIT" || application.stage === "TECHNICAL") && (
            <div className="border rounded-lg p-3 space-y-2">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button className="btn-secondary" onClick={() => scheduleInterview(application.id)}>
                  Generate Invite
                </button>
              </div>
            </div>
          ...
          {application.interviews?.map((iv: any) => (
            <div key={iv.id} className="border rounded-lg p-3 text-sm">
              <p className="font-medium">
                {iv.round} —{" "}
                {iv.scheduledAt ? new Date(iv.scheduledAt).toLocaleString("en-GB") : "Not scheduled"}
              </p>
          ...
          <div>
            ...
            {application.emailLogs?.length === 0 && (
              <p className="text-xs text-slate-400">No emails yet.</p>
            )}
              ...
              {application.emailLogs?.map((e: any) => (
                <div key={e.id} className="border rounded-lg p-2 text-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`badge ${e.status === "sent" ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"}`}>
                      {e.status}
                    </span>
                  </div>
